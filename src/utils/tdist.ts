...
// 由 newbee-mall-vue3-app 的 src/common/js/utils.js 拆分而来：
// 全国省市区数据及层级查询函数。数据量较大（约 150 KB），独立成模块，
// 仅在地址编辑等页面按需引入，避免打进首屏主包。

/** 区划条目：[名称, 上级区划编码] */
export type DistEntry = [name: string, parentCode: string]
...
/** 全国省市区数据，键为 6 位区划编码（省 XX0000 / 市 XXYY00 / 区县 XXYYZZ） */
export const tdist: Record<string, DistEntry> = {
  210184: ["沈北新区", "210100"],
  210185: ["其它区", "210100"],
  420381: ["丹江口市", "420300"],
  420383: ["其它区", "420300"],
...
// 以下三个查询函数原是挂在 tdist 对象上的方法（tdist.getLev1() 等），
// 现改为独立的具名导出，数据与逻辑分离，也便于按需引用。

const pad2 = (n: number) => String(n).padStart(2, '0')

/** 省级列表（编码形如 XX0000） */
export function getLev1(): DistOption[] {
  const list: DistOption[] = []
  for (let i = 1; i < 100; i++) {
    const code = `${pad2(i)}0000`
    const entry = tdist[code]
    if (entry) list.push({ id: code, text: entry[0] })
  }
  return list
}

/** 某省下的市级列表（编码形如 XXYY00） */
export function getLev2(provinceCode: string): DistOption[] {
  if (!provinceCode) return []
  const prefix = provinceCode.slice(0, 2)
  const list: DistOption[] = []
  for (let i = 1; i < 100; i++) {
    const code = `${prefix}${pad2(i)}00`
    const entry = tdist[code]
    if (entry) list.push({ id: code, text: entry[0] })
  }
  return list
...
/** 某市下的区县列表（编码形如 XXYYZZ） */
export function getLev3(cityCode: string): DistOption[] {
  if (!cityCode) return []
  const prefix = cityCode.slice(0, 4)
  const list: DistOption[] = []
  for (let i = 1; i < 100; i++) {
    const code = `${prefix}${pad2(i)}`
    const entry = tdist[code]
    if (entry) list.push({ id: code, text: entry[0] })
  }
  return list
